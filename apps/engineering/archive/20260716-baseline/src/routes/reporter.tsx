...
export const Route = createFileRoute("/reporter")({
  head: () => ({ meta: [{ title: "ผู้แจ้ง — ระบบแจ้งซ่อม" }] }),
  component: () => (<RequireRole role="reporter"><ReporterPage /></RequireRole>),
});
...
function ReporterPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (created?.id) {
        syncSheet({ data: { job_id: created.id } }).catch((e) => console.error("sheet sync", e));
      }
      await load();
    } catch (err) {
  ...
  const reviewJob = async (id: string, accept: boolean) => {
    ...
    if (error) toast.error(error.message);
      ...
      syncSheet({ data: { job_id: id } }).catch((e) => console.error("sheet sync", e));
      await load();
    }
  };
