...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      if (!mounted) return;
      // Only react to real identity transitions. Ignore TOKEN_REFRESHED / INITIAL_SESSION
      // which fire on tab focus and would otherwise flash the loading state and reset
      // any child page state (e.g. active tab in leader view).
