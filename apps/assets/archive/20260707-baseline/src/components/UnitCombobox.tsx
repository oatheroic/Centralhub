 ...
 * - placeholder "หน่วย เช่น เครื่อง" สีแดง
 */
...
function AdminPanel({ opts, reload, onPick }: { opts: Opt[]; reload: () => void; onPick: (v: string) => void }) {
  ...
  async function add() {
    ...
    if (existing) {
      if (existing.is_active) return toast.error("มีหน่วยนับนี้อยู่แล้ว");
  ...
  async function save(oldVal: string) {
    ...
    if (v !== oldVal && opts.some((o) => o.value === v)) return toast.error("มีหน่วยนับนี้อยู่แล้ว");
    const { error } = await supabase
      .from("dropdown_options")
      .update({ value: v })
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button type="button" variant="outline" size="icon" title="จัดการหน่วยนับ (Admin)">
          <Settings className="h-4 w-4" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-80" align="end">
        <div className="space-y-2">
          <Label className={labelCls}>เพิ่มหน่วยนับ</Label>
          <div className="flex gap-2">
            <Input
              value={adding}
          ...
          {opts.length > 0 && (
            <>
              <div className="border-t pt-2 mt-2">
                <Label className={labelCls}>จัดการหน่วยนับ ({opts.length})</Label>
...
export async function ensureUnitOption(name: string) {
  const v = (name ?? "").trim();
  if (!v) return;
  const { data } = await supabase
    .from("dropdown_options")
